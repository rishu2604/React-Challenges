...
export default function WelcomePage() {
  ...
  // scrollX for horizontal scrolling
  // scrollYProgress for relative vertical scrolling (returns values from 0 to 1)

  // First argument: Takes a value that should be transformed 
  // Second argument: Array of breakpoints
  // Third argument: array of values on which the breakpoints must be transform to
  const opacityCity = useTransform(scrollY, [0, 200, 350, 500], [1, 0.4, 0.4, 0]);
  const yCity = useTransform(scrollY, [0, 200], [0, -100]);

  const opacityHero = useTransform(scrollY, [0, 300, 500], [1, 0.8, 0]);
  const yHero = useTransform(scrollY, [0, 200], [0, -150]);

  const scaleText = useTransform(scrollY, [0,300], [1, 1.5]);
  const yText = useTransform(scrollY, [0, 200, 350, 500], [0, 50, 50, 300]);
  return (
    <>
